...
object GradientMaterialSpec: Spek({
  context("when rendering the pattern") {
    it("should linearly interpolate between colors") {
      val pattern = GradientMaterial(SolidColor(Color.WHITE), SolidColor(Color.BLACK))

      assertEquals(Color(0.75f, 0.75f, 0.75f), pattern.patternAt(Point(0.25f, 0f, 0f)))
      assertEquals(Color( 0.5f,  0.5f,  0.5f), pattern.patternAt(Point( 0.5f, 0f, 0f)))
      assertEquals(Color(0.25f, 0.25f, 0.25f), pattern.patternAt(Point(0.75f, 0f, 0f)))
    }
  }
})